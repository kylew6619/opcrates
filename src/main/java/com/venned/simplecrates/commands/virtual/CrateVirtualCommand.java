package com.venned.simplecrates.commands.virtual;
...
public class CrateVirtualCommand implements CommandExecutor {


    CrateVirtualManager manager;
    CrateBlockManager crateBlockManager;
    EditChances editChances;
    PlayerManager playerManager;

    public CrateVirtualCommand(CrateVirtualManager manager, EditChances editChances, CrateBlockManager crateBlockManager, PlayerManager playerManager) {
    ...
    public boolean onCommand(CommandSender sender, Command command, String s, String[] args) {
        if(sender instanceof Player player) {
            if(args.length == 0) {
                List<String> info = Arrays.asList("&c&lInfo Crate Virtual",
                        " ",
                        "&d&lSub Commands: ",
                        " ",
                        "&c- &7create <name> <display_name> -> create a crate virtual ",
                        ...
                        "&c- &7edit <name> -> You can edit the crate virtual",
            ...
            switch (args[0]) {
                case "create" -> {
                    ...
                    CrateVirtual crate = new CrateVirtual(name, new ArrayList<>(), displayName);
                ...
                case "setkey" -> {
                    ...
                    String name = args[1];
                    CrateVirtual crate = manager.getCrates().stream()
                ...
                case "give" -> {
                    ...
                    String name = args[1];
                    CrateVirtual lootBox = manager.getCrates().stream()
                            .filter(n->n.getName().equalsIgnoreCase(name)).findFirst().orElse(null);
                    if(lootBox == null) {
                        player.sendMessage("§c§l(!) §cThere is no such Crate");
                        return true;
                ...
                case "givekey" -> {
                    ...
                    int current = lootBox.getOwned().getOrDefault(player.getUniqueId(), 0);
                    lootBox.getOwned().put(player.getUniqueId(), current + amount);
                    playerFind.sendMessage("§c§l(!) §dCrate Key " + name + "  was given to you");
                    break;
                }
            }
